...
type Driver = { id: string; user_id: string | null; email?: string | null; name: string; username: string; phone: string; status: string; profile_image_url?: string | null; rating?: number; rating_count?: number; approval_status?: string | null; suspended_until?: string | null; suspension_reason?: string | null; branch_id: string | null; bank_name?: string | null; bank_account_number?: string | null; bank_account_holder?: string | null };
type Delivery = {
  id: string;
  order_id: string;
  ...
  return (
    <div className="min-h-screen bg-muted/40 pb-20">
              ...
              <div key={d.id} className="space-y-2 rounded-2xl border border-muted/30 p-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  ...
                  <button type="button" onClick={() => void openDriver(d)} className="min-w-0 flex-1 text-left">
                    <div className="font-semibold text-sm">{d.name}</div><div className="text-xs text-brand">Customer username: {d.username}</div>
      ...
      {selectedDriver && <div className="fixed inset-0 z-[90] grid place-items-center overflow-y-auto bg-black/60 p-4" role="dialog" aria-modal="true" aria-label={`${selectedDriver.name} driver information`} onClick={() => setSelectedDriver(null)}><div className="my-6 w-full max-w-2xl rounded-3xl border bg-background p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2"><Info label="Customer username" value={selectedDriver.username} /><Info label="Email" value={selectedDriver.email} /><Info label="Phone" value={selectedDriver.phone} /><Info label="Branch" value={selectedDriver.branch_id ? branches[selectedDriver.branch_id]?.name : null} /><Info label="Approval" value={selectedDriver.approval_status} /><Info label="Bank" value={selectedDriver.bank_name} /><Info label="Account number" value={selectedDriver.bank_account_number} /><Info label="Account holder" value={selectedDriver.bank_account_holder} /><Info label="Applied" value={selectedDriverApplication ? new Date(selectedDriverApplication.created_at).toLocaleString() : null} /><Info label="Identity number" value={selectedDriverApplication?.id_number} /><Info label="Licence number" value={selectedDriverApplication?.student_number} /><Info label="Application status" value={selectedDriverApplication?.status} /><Info label="Admin notes" value={selectedDriverApplication?.admin_notes} /></div>
      ...
      {selectedApplicationReview && <div className="fixed inset-0 z-[95] grid place-items-center overflow-y-auto bg-black/60 p-4" role="dialog" aria-modal="true" aria-label={`${selectedApplicationReview.name} driver application`} onClick={() => setSelectedApplicationReview(null)}><div className="my-6 w-full max-w-2xl rounded-3xl border bg-background p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2"><Info label="Customer username" value={selectedApplicationReview.username} /><Info label="Email" value={selectedApplicationReview.email} /><Info label="Phone" value={selectedApplicationReview.phone} /><Info label="Branch" value={selectedApplicationReview.branch_id ? branches[selectedApplicationReview.branch_id]?.name : null} /><Info label="Identity number" value={selectedApplicationReview.id_number} /><Info label="Licence number" value={selectedApplicationReview.student_number} /><Info label="Bank" value={selectedApplicationReview.bank_name} /><Info label="Account number" value={selectedApplicationReview.bank_account_number} /><Info label="Account holder" value={selectedApplicationReview.bank_account_holder} /><Info label="Submitted" value={new Date(selectedApplicationReview.created_at).toLocaleString()} /><Info label="Admin notes" value={selectedApplicationReview.admin_notes} /></div>
