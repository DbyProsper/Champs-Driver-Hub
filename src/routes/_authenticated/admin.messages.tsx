...
type Driver = { id: string; name: string; profile_image_url: string | null; branch_id: string | null };
type OrderChat = { id: string; order_number: string; driver_id: string; customer_name: string; workflow_status: string; branch_id: string };
...
function MessagesPage() {
  ...
  const load = useCallback(async () => {
    const [{ data: driverRows }, { data: orderRows }, { data: branchRows }] = await Promise.all([
      supabase.from("drivers").select("id,name,profile_image_url,branch_id").order("name"),
      (supabase as any).from("orders").select("id,order_number,driver_id,customer_name,workflow_status,branch_id").not("driver_id", "is", null).order("created_at", { ascending: false }).limit(200),
  ...
  const rows = useMemo(() => drivers.filter((driver) => scope === "both" || driver.branch_id === scope).map((driver) => ({ driver, orders: orders.filter((order) => order.driver_id === driver.id && (scope === "both" || order.branch_id === scope)) })), [drivers, orders, scope]);
