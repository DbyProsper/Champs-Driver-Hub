...
export function PasswordChangeForm({ title = "Change password" }: { title?: string }) {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(({ data }) => {
      if (!active) return;
      setRequiresCurrentPassword(data.user?.app_metadata?.provider !== "google");
    });
    return () => { active = false; };
  }, []);
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (requiresCurrentPassword && currentPassword === newPassword) return toast.error("Choose a password different from your current password");
    setBusy(true);
    try {
  ...
  return (
    <section className="rounded-2xl border bg-card p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">{requiresCurrentPassword ? "Confirm your current password before choosing a new one." : "You signed in with Google. Create a password to also sign in with your email address."}</p>
      <form onSubmit={submit} className="mt-3 grid gap-2">
        {requiresCurrentPassword && <PasswordInput label="Current password" value={currentPassword} onChange={setCurrentPassword} visible={visible} />}
        ...
        <div className="mt-1 flex flex-wrap items-center justify-between gap-2">
          ...
          <button disabled={busy || (requiresCurrentPassword && !currentPassword) || !newPassword || !confirmPassword} className="rounded-full bg-brand px-4 py-2 text-xs font-bold text-brand-foreground disabled:opacity-50">{busy ? "Saving…" : requiresCurrentPassword ? "Change password" : "Set password"}</button>
        </div>
      </form>
    </section>
  );
